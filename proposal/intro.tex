...
%
These applications are bottlenecked by the performance of an underlying core set of data structures, including filters~\cite{cite-place-where-bottleneck}, hash tables~\cite{same}, locality sensitive hashing, compressed string indexes \mfc{fill in the rest from the remainder of intro, once we've read it.}.
%

In this project, we aim to build high-performance and scalable data-analysis pipelines for computational biology applications.  To do so, we will address the main computational bottleneck of existing systems by developing new massively parallel and distributed data structures and algorithms for core computational biology data-processing tasks.
For concreteness, we consider the following two tools
\mab{Replace tools with more specific word. Are these tools?}
that are used throughout computational biology.

\mab{say, explicitly, and or provide evidence that these are actually bottlenecks.  maybe here or maybe in the applications}

\prashant{1. We need to be faster and bigger. 2. There's a relatively small set of data structure that are bottleneck for multiple applications. 3. If we make these data structures faster and smaller it will have widespread impact. 4. We have additional source of computational power (GPU) that PIs already have experience with. 5. GPUs are fast but are constrained in space. 6. Therefore, we need new data structures and algorithms that are space efficient and scalable to the CPU RAM to speed up these applications. 7. We are the perfect team to this. We already have experience with GPUs, core algorithms and data structures, computational biology, and everyone in the team has worked together for years.}
...
Some approaches~\cite{kim2016centrifuge} employ compact string indexes such as the FM-index~\cite{ferragina2000opportunistic} to perform sequence comparison to determine exact matches among the pruned-down samples. \mfc{which of these are bottlenecks?}
