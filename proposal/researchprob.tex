...
We envision a single performance model for distributed GPU computation.  With today's GPUs, however, a more realistic goal is a small set of algorithmic models to capture the different bottlenecks that GPUs face.
\end{rproblem}

Although there has been excitement about striving for a single algorithmic GPU model among parallel computing researchers~\cite{SitchinavaWe13, MaChAg14,xxx}, it is unlikely that there is a single performance model that works for all computations.
...
Currently, Mantis and other raw sequecence search indexes are not able to index a large fraction of the SRA.
[Summary sentence talking a little bit about the state of the art and one about the challenges.]
\label{rprob:peppermint}
\end{rproblem}
