...
%
Furthermore, so far GPUs are not utilized to scale up sequence-to-sequence alignment and sequence-to-graph alignment. Existing data structures are designed for single-node CPUs. Also, sequence-to-graph alignment algorithms do not scale out to distributed memory to support large-scale pangenomics graphs. \rob{This last paragraph sort of comes out of nowhere. We are talking about string indexes, and then all of a sudden we are talking about alignment and graph alignment. I think we need to make this fit better if we want to keep it.}
...
We plan to extend FM-index, P-BWT\draft{, and SBWT} data structures using a new framework for dynamic data structures for string processing~\cite{Nicola17,Farzan2011}. Our plan is to make the underlying bit vector representation dynamic and support efficient rank and select queries on dynamic bit vectors~\cite{Raman2001}.
