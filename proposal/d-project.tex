...
We aim to develop new data structures and algorithms will have wide applicability in computation biology and beyond.  Consider for concreteness we consider the following toolkit used throughout computational biology.  
...
Sequence-to-sequence alignment and sequence-to-graph alignment
...
\begin{itemize}[leftmargin=*]
\item {\bf Raw sequence search.} In raw sequencing data, singleton \kmers are most likely caused by sequencing errors, yet they make up a large fraction of the data~\cite{xxxx}. 

\prashant{Expand this app.}
...
Methods for taxonomic classification of metagenomic data~\cite{cite-something} often use hash tables to index the \kmer content of samples and quickly compare samples to prune down the search space. 
Furthermore, to save space many solutions~\cite{cite-something} also employ lossy sketches (e.g., locality sensitive hashing (LSH)~\cite{cite-something}) to represent metagenomic data and perform similarity computations over sketches. \mfc{lossy or approximate?}
...
\caption{Applications -- data structures -- features. \prashant{Add another layer tool.}}
\label{fig1}
\end{wrapfigure}
...
\prashant{What are existing tools and their bottlenecks.}\\
\prashant{K-mer tools: Jellyfish, KMC3, Squeakr, etc.}\\
\prashant{Taxonomic class tools: Karaken2, MetaSeqOthello, etc.}\\
\prashant{Pangenomics tools: VG, Variantstore, Minigraph, etc.}\\
\prashant{Raw sequence search tools: Mantis, SSBT, COBS, etc.}
...
\mfc{maybe we can say something from The End of Moore's Law paper?}
...
GPUs are used in some computational biology applications to speed up \kmer counting~\cite{xxx}, local assembly~\cite{xxx}, read-to-contig mapping~\cite{xxx}.
Give examples.
...
These limitations are critical for building scalable computational biology applications. \mfc{add details}
