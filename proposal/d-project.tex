...
due to high-throughput sequencing technology (HTS)~\cite{cite-sequencing-technology}, which is already producing
petabyte-scale datasets~\cite{cite-petabyte-scale}. Many applications in computational biology (\kmer
...
build high-performance and scalable data analysis-pipelines for computational-biology applications. \mfc{something about novel algorithms and target hardware}

\mfc{How does this fit into the story here?} As we explain (see \S{xxx} and \S{xxx}), the key technology \mfc{why is this a technology?}  in these pipelines is high-performance, compact, dynamic, and scalable data structures. \mfc{we're describing the problems we are attacking here, not the solutions} For example:
...
Existing tools use both \mfc{in this context, I don't think of filters as lossy.  maybe approximate vs exact?} lossy and lossless data structures (e.g., filters vs.\ compact \mfc{so solutions require compactness?  or only good solutions?  Do the filters not need to be compact?}  hash tables) to construct and store \kmer indexes~\cite{xxxx}.
For example, in raw sequencing data, singleton \kmers are erroneous \mfc{I don't know what erroneous means.  does it mean "sampled from a read region with coverage 1"?  why is that erroneous?  I suppose that they could also be erroneous, but just the word erroneous is confusing} and make up a large fraction of the data~\cite{xxxx}. \kmer analysis tools such as  XXX use a compact filter to identify singleton \kmers and use a hash table to maintain the frequency count and associated metadata (e.g., prefix-suffix extension, read id, etc) about the \kmers~\cite{xxxx}.
...
Methods for taxonomic classification of metagenomic data~\cite{cite-something} often use hash tables to index the \kmer content of samples and quickly compare samples to prune down the search space. \mfc{so this is not a distinct application from k-mer analysis.}
Furthermore, to save space many solutions~\cite{cite-something} also employ lossy sketches (e.g., locality sensitive hashing (LSH)~\cite{cite-something}) to represent metagenomic data and perform similarity computations over sketches. \mfc{lossy to me means false negatives.  Isn't this false positive, so approximate?}
...
\mfc{raw sequence analysis is one of the applications in the figure}
...
\prashant{Need a sentence that connects to data structures.}
\mfc{maybe we can say soemthing from The End of Moore's Law paper?}

\setlength\intextsep{0pt}
\begin{wrapfigure}[18]{R}{3.0in}
\centering
\includegraphics[width=3.0in]{images/SRA_data_growth.png}
...
GPUs are increasingly used in high-performance computing (HPC) \mfc{where did HPC come from?  Stay on topic of comp bio} because they offer a discontinuous amount \mfc{what's a discontinuous amount? isn't any amount discrete and thus discontinous?} of compute power and low cost compared to CPUs.
GPUs are fast and efficient when computations \mfc{not simple and straightforward.  rather "conform to the memory and parallelism requires of GPUs"} are simple and straightforward and data fits in device RAM (e.g., many examples).
\prashant{add more examples.}

GPUs are used in some computational biology  applications to speed up data processing~\cite{xxx} \mfc{doesn't data processing cover all other applications?  everythign is data processing}, sequence alignment~\cite{xxx}, read-to-contig mapping~\cite{xxx}.
Give examples.

However, GPUs are not used in most computational biology pipelines because of GPU limitations:
...
These limitations affect the capabilities of GPU data structures: \mfc{the proceeding is vacuously true.  We might was well write: These limitations of GPUs are limitations of GPUs.}
most existing GPU data structures do not support dynamic resizes and are statically allocated; pointer-based data structures such as trees and tries are do not achieve high performance on GPUs; \mfc{what's a variable length data structure?  Isn't it data structures on variable-length data?  And isn't it really hard-to-align data, such as string or vectors, which have variable length?} variable-length data structures such as strings and vectors are not available; data structures do not scale out of GPU device RAM.
