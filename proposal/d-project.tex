...
\caption{Sequence read archive data growth over years. \mab{this is such a great figure. We should have an expanded caption show that to figure Plus caption speaks by itself. We should also make the figure smaller, because space is at a premium (and also because we want it to look like spaces at a premium).}}
\label{fig1}
\end{wrapfigure}
...
\begin{rproblem}[\bf asymptotic performance models for GPU computations]
We envision of world in which there will be a single performance model for GPU computation.  With today's GPUs, a more realistic goal is a small set of algorithmic models to capture the different bottlenecks that GPUs face.
\end{rproblem}

Although there has been excitement about such an algorithmic vision among parallel computing researchers~\cite{cite-stuff}, it is unlikely that there is a single performance model that works for all computations.
...
Thus,  part of our task, for each problem that we develop, is to determined what is the central computational bottleneck, and use and algorithmic model that captures that bottleneck.

- PRAM. Alas, these algorithms are often quite good, despite being far from the GPU architecture. Syncs and global memory are the bottlenecks, but if the PRAM somehow behaves locally it can often work well enough.

-Parallel external memory model (PEMM), or some variant: seems elegant-ish, and the idea of having increasing number of processors sharing increasing caches which have increasing cache lines is undoubtedly the right big picture. But not much has been done, and I feel these won't give you immediately super-good GPU algorithms
