...
As we explain (see \S{xxx} and \S{xxx}), the key technology in these pipelines is high-performance, compact, dynamic, and scalable structures. For example: 

\begin{itemize}[leftmargin=*]
%[noitemsep, leftmargin=*]
    
    \item {\bf \Kmer analysis} refers 
to counting length-$k$ subsequences (called \kmers)
in raw sequencing data and associating auxiliary information (e.g.,
prefix-suffix extension, read id, etc) with each \kmer. \Kmer analysis is the
very first step in numerous computational biology pipelines, e.g., error
correction, de Bruijn graph construction, raw sequence search, digital
normalization, comparative genomics, genomics assembly, transcript
quantification, taxonomic classification of metagenomic reads, etc. Existing
tools for \kmer analysis~\cite{cite-something} use a filter to weed out
singleton \kmers and a hash table to count and map \kmers occurring twice or
more.

\prashant{k-mer data sets are skewed. A major fraction of the k-mers are singletons which are erroneous. Tools use filters to identify singleton k-mers. They only store non-singleton k-mers in the hash table which have a high space cost. k-mer analysis is a way represent a raw sequencing data in form of its k-mer content. Applications perform downstream analysis on sequencing datasets by comparing their k-mer contents.}

    \item {\bf Taxonomic classification} refers  to Identifying the microbial taxon or taxa
present in large-scale  metagenomic datasets coming from complex biological and
environmental samples. Assigning taxonomic labels to sequencing reads is an
important part of many computational genomics pipelines for metagenomics
projects. Numerous methods~\cite{cite-something} for taxonomic classification of
metagenomic data have been developed that often use hash tables to index the
\kmer content of samples and quickly compare samples to prune down the search
space. They further employ compact string indexes such as FM-index and
compressed suffix array (CSA) to perform sequence comparison to determine exact
matches.

    \item {\bf Pangenomics} refers to storing the genome of a species as a sequence graph
consisting of genomes of multiple individuals instead of a single linear
reference. A primary goal of pan-genomic variation analysis is to avoid biases
that arise when treating a single genome as the reference when identifying or
comparing variants across samples. Existing tools for  constructing a pangenomic
graph use a combination of succinct data structures (compressed bit vectors) and
space-efficient and dynamic hash tables and string data structures.
