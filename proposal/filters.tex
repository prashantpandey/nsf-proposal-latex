...
%
de Bruijn graphs are often large enough that they do not fit in main memory. They also change over time to support adding \kmers from new sequences and removing existing \kmers to weed out erroneous sequences.
...
Dynamic adaptive maplets can be used to build a compact, error-free, and fast representation for de Bruijn graphs. Adaptive maplets would, in theory, take smaller space compared to hash tables and they can adapt during queries to avoid any false positives. Adaptive filters are quite appropriate for representing and querying de Bruijn graphs as the set of queries is known in advance. Furthermore, dynamic adaptive filters can support \kmer additions and deletions from the de Bruijn graph. \rob{Two thoughts about this. First: We keep saying that the set of queries is known in advance.  This is true, but only in the case of a navigational representation.  That is, if someone wishes to use our representation for membership queries, we don't know the set of queries in advance, only if they start at some valid \kmer and are then allowed to navigate the graph. Second: There is some work on dynamic succinct de Bruijn graphs e.g. \url{https://academic.oup.com/bioinformatics/article/37/14/1946/5848003}.  Should we address this? Also, we should think if there is a good reason to approach this particular problem (dBG representation) using dynamic adaptive filters rather than trying to create dynamic versions of existing succinct representations (which are membership representations and not just navigational)}
