...
and scientific computing\mfc{cite}. Although CPU performance is increasing at
2--25\%/year and single-node RAM sizes are increasing at 2--11\%/year, genomics
data, for example, is likely to double in size every 1.5 years~\cite{kodama2012sequence} \mfc{, and blah data is likely to blah}.
...
scale (e.g., training deep neural networks)~\mfc{cite} and complex dynamic GPU data
structures not at scale (including many contributions by the PIs of this
proposal)~\mfc{cite}. The success of both of these thrusts of work gives us confidence
...
computations across computational biology, \rob{do we actually propose pipelines in these other domains?}\textcolor{red}{machine learning, databases, and
...
We believe the answer to \mfc{which?} these challenges is a collection of  memory-efficient,
