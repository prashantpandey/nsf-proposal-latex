...
\begin{itemize}
    \item set up a target problem of data analystics, where we are going to use comp bio as an example
    \item talk about how many data pipelines end up using the same basic data structures.  forward pointer to how we work this out in detail in section 3 for 3 seamingly different/unrealted comp bio problems
    \item so the topic of this proposal is high-throughput HPC versions of these data structures 
    \item argue that things are now at a scale where this will require GPUs
    \item then talk about how GPUs have been great for regular computations, but these data structures are irregular
    \item talk about the technical challenges of building these data structures at scale
    \item talk about why we will succeed/what has change in the world so we can succeed
    \item talk about how we have clear application targets that will validate that our work is useful on actual data pipelines
\end{itemize}
...
\mfc{this feels appologetic.  Is there a better table that project's novelties?}
