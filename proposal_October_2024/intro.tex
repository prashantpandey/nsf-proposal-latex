...
discovery, etc.  \mab{What can we cite here?} \john{I don't like the word ``exponentially'' at all.} \rob{replaced it with ``rapidly.''} As a result, increasingly complex
and large-scale analyses are employed to gain insights into this data. 
\mab{I cannot tell whether I like this previous sentence, but it seems out of place.}
For
...
storage. The ability to scale complex data analyses to these vast quantities of data
arguably rank among the biggest challenges in the next decade.

\mab{How about ``As we argue in this proposal, a large number of diverse applications are often bottlenecked on a surprisingly small number of core data structures.'' 
What's there now isn't really doing it for me.}
...
\mab{Idea to communicate somewhere nearby:  
any improvements to these data structures would have a disproportionately large positive impact on many applications. }
...
\mab{This last sentence seems a little weak. Not sure if it's worth fixing.}

\mab{Here is a place to put one or two sentences about what we do, emphasizing the coolness. 
E.g., (1) vertically oriented/end-to-end solution where we show how a small set of new and improved data structures can take advantage of faster hardware (GPUs) to make a range of  applications faster. Our targets are three (how many?) related applications in comp.\ bio. Can we say why these are universal---tons of data, space and CPU intensive, etc.}
...
\mab{We don't really say why these core data structures are so fundamental. I think that there's a nondefensive way to do, as we link with comp bio. }
...
\mab{One of the grand challenges in GPU computing is...}
What has been more challenging, and the
focus of significant research effort \mab{(including by three of the PIs~\cite{xxx,xxx,xxx})} over the past decade, has been
...
\mab{By mentioning the contributions of the PIs only for the second piece and not the first piece, it makes me think ``oh, the PIs only have expertise in one of the two thrusts.''}
...
\rob{it also feels redundant --- we've said almost exactly the same earlier.}
